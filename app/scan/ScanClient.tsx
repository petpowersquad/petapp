...
function ScanPageInner({ pets }: ScanClientProps) {
  const searchParams = useSearchParams();
  const previewParam = searchParams.get("preview");
  const petParam = searchParams.get("petId");

  const [selectedPetId, setSelectedPetId] = useState<string>(petParam ?? "");
      ...
      <div className="mx-auto max-w-4xl space-y-8">
        ...
        <div className="text-center space-y-1">
          ...
          <p className="text-sm text-text-muted max-w-md mx-auto">
            Upload a clear close-up photo of your pet&apos;s symptom (skin,
            eyes, ears, etc.) for instant triage suggestions.
          </p>
        </div>
        ...
        {!scanResult ? (
          <Card className="border-border shadow-sm overflow-hidden">
            <CardHeader className="bg-primary text-white pb-6 pt-8 text-center gap-0 -mx-6 -mt-6 mb-6">
              ...
              {/* Pet selector — inside card header */}
              <div className="mx-auto w-full max-w-xs mt-1">
                {pets.length === 0 ? (
                  <div className="flex items-center justify-center gap-2 rounded-lg border border-white/20 bg-white/10 px-3 py-2 text-xs text-white/70">
                    <PawPrint className="h-4 w-4 shrink-0" />
                    No pets yet — add one from the dashboard
                  </div>
                ) : (
                  <Select
                    value={selectedPetId}
                    ...
                    itemToStringLabel={(v) =>
                      pets.find((p) => p.id === v)?.name ?? String(v)
                    }
                  >
                    <SelectTrigger
                      className="w-full border-white/30 bg-white/10 text-white placeholder:text-white/60 hover:bg-white/20 focus:ring-[var(--accent-primary)] focus:border-[var(--accent-primary)] [&>span]:flex [&>span]:items-center [&>span]:gap-2"
                      aria-label="Select pet to scan"
                    >
                      <PawPrint className="h-4 w-4 shrink-0 text-[var(--accent-primary)]" />
                      <SelectValue placeholder="Select a pet…" />
                    </SelectTrigger>
                    <SelectContent className="border-[var(--border-default)] bg-[var(--bg-surface)]">
                      {pets.map((pet) => (
                        <SelectItem
                          key={pet.id}
                          value={pet.id}
                          className="text-[var(--text-primary)] focus:bg-[var(--accent-primary)]/10 focus:text-[var(--text-primary)]"
                        >
                          {pet.name}
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex justify-end gap-3">
                    ...
                    <Button
                      variant="secondary"
                      onClick={triggerScan}
                      disabled={isScanning || !selectedPetId}
          ...
          <Card className="border-border shadow-md overflow-hidden animate-in fade-in zoom-in-95 duration-300">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <CardDescription className="text-white/60">
                      {selectedPet
                        ? `${selectedPet.name} · Generated just now · ID: #TR-94827`
                        : "Generated just now · ID: #TR-94827"}
                    </CardDescription>
                  </div>
                </div>
            ...
            <CardContent className="p-6 space-y-6 -mb-6">
              <div className="flex gap-4 p-4 rounded-xl border border-border bg-slate-50/50">
                ...
                <div className="space-y-1">
                  <div className="text-sm font-semibold text-primary">
                    {selectedPet ? selectedPet.name : "Uploaded Specimen"}
                  </div>
                  <div className="text-xs text-text-muted">
                    Analyzing: Skin irritation &amp; hair loss near right ear.
                  </div>
                  <div className="text-[10px] text-emerald-600 font-semibold bg-emerald-50 px-2 py-0.5 rounded-full inline-block border border-emerald-100">
                    Confidence: 89%
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3 text-xs leading-relaxed text-text-muted">
                  <div className="flex gap-3 items-start">
                    ...
                    <p>
                      <strong className="text-primary">Monitor irritation closely:</strong>{" "}
                      Clean the area gently with warm water. Do not apply human
                      creams or ointments which could worsen irritation.
                    </p>
                  </div>
                  <div className="flex gap-3 items-start">
                    <span className="h-2 w-2 rounded-full bg-state-warning shrink-0 mt-1.5" />
                    <p>
                      <strong className="text-primary">Prevent scratching:</strong>{" "}
                      Use an Elizabethan collar (cone) if necessary to prevent
                      the pet from scratching or licking the area.
                    </p>
                  </div>
                  <div className="flex gap-3 items-start">
                    <span className="h-2 w-2 rounded-full bg-state-success shrink-0 mt-1.5" />
                    <p>
                      <strong className="text-primary">Schedule vet checkup:</strong>{" "}
                      We recommend scheduling an appointment within 3–5 days for
                      a proper diagnosis.
                    </p>
                  </div>
                </div>
              ...
              <div className="rounded-xl border border-red-100 bg-red-50/50 p-4 text-xs text-red-900 leading-relaxed">
                <div className="flex items-center gap-1.5 font-bold mb-1.5">
                  <HelpCircle className="h-4 w-4 text-destructive" /> Medical Disclaimer
                </div>
                This scan was processed by a machine learning model and does not
                constitute veterinary diagnostics or professional medical advice.
                Always consult a licensed veterinarian in person.
              </div>
...
export default function ScanClient({ pets }: ScanClientProps) {
  return (
    <Suspense
      fallback={
